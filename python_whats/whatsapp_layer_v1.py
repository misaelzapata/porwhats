...
class WhatsAppLayer(YowInterfaceLayer):
    ...
    def onMessage(self, message):
        ...
        if message.getType() == "text":
            ...
            #'message_type': 'media', 'media_type': messageProtocolEntity.getMediaType(), 'from_user': messageProtocolEntity.getFrom(False), 'caption' : messageProtocolEntity.getCaption() }
            '''
            post_id = str(uuid.uuid4())     
            name = ''.join(os.path.splitext(os.path.basename(urlparse.urlsplit(message).path)))           
            data = {}
            data["name"]=name
            ...
            '''
            print message.getMediaType()
            print message.getFrom(False)
            print message.getCaption()
            messageOut['media_type'] = message.getMediaType()
            messageOut['from_user'] = message.getFrom(False)
            messageOut['caption'] = message.getCaption()
            messageOut['message_type'] = message.getType()
            
            post_data = messageOut
            try:
                r.publish('new_message', json.dumps(post_data))
            except:
                logging.error("Could not send to redis", exc_info=True)                
            try:
                result = db.received.insert_one(messageOut)
            except:
                logging.error("Could not send to mongo", exc_info=True)                
        else:
    ...
    def getDownloadableMediaMessageBody(self, message):
        ...
        data = {'media_type' : message.getMediaType(),
...
#    stack = YowsupEchoStack(credentials=CREDENTIALS)
